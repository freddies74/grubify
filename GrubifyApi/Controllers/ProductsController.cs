...
namespace GrubifyApi.Controllers
{
    ...
    {
        private static readonly List<FoodItem> Products = new()
        {
        ...
        [HttpGet]
        public ActionResult<IEnumerable<FoodItem>> GetProducts()
        {
            return Ok(Products);
        }

        /// <summary>
        /// Returns a product by its numeric ID.
        /// Returns 400 Bad Request for non-integer or non-positive identifiers (e.g. "phpPhotoAlbum", "bad397")
        /// to prevent malformed probe traffic from reaching the data layer and causing invalid-input errors.
        /// </summary>
        [HttpGet("{id}")]
        public ActionResult<FoodItem> GetProduct(string id)
        {
            if (!int.TryParse(id, out int productId) || productId <= 0)
            {
                return BadRequest(new { error = "Invalid product identifier. Must be a positive integer.", value = id });
            }
        ...
        public ActionResult<IEnumerable<FoodItem>> GetProductsByCategory(string category)
        {
            var items = Products.Where(f =>
